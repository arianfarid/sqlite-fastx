...
fn main() {
    ...
    let min_len: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(100_000);
    let max_len: usize = args.get(3).and_then(|s| s.parse().ok()).unwrap_or(500_000);

    let fa_path = "bench_wide.fa";
    let fai_path = "bench_wide.fa.fai";
    let nofai_path = "bench_wide_nofai.fa";
    ...
    for i in 0..count {
        ...
        // FAI: name, length, seq_offset, bases_per_line, bytes_per_line
        // Single-line sequences: bases_per_line == length, bytes_per_line == length + 1
    ...
    eprintln!("Copied to {nofai_path} (no FAI index, for no-index benchmarks)");
}
